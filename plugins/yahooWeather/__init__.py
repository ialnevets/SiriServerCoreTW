...
class yahooWeather(Plugin):
    ...
    def forcastWeatherAtLocation(self, speech, language, regex):
        string = re.findall(u".*天氣於(?P<location>[\s\S]+?)$",speech)
        if string == []:
            string = re.findall(u"(?P<location>[\s\S]+?)的天氣.*",speech)

        self.logger.info(u"# Weather # {0} ##########".format(string))
        if string == []:
            #####
            ...
            self.logger.info(u"## reverseLookup ## {0} ##########".format(reverseLookup))
            ...
            #####
            self.showWaitPlease(language)
            query = "select woeid from geo.places where text=\"{0}\"".format(string)
            lookup = "http://query.yahooapis.com/v1/public/yql?q={0}&format=xml".format(urllib.quote(query.encode("utf-8")))
            self.logger.info(u"## forcastWeatherAtLocation ## {0} ##########".format(lookup))
