...
class phonecallPlugin(Plugin):
    ...
    @register("en-US", u"(打給(.*))|(.*撥.*給(.*))|(.*電話(.*))")
    def makeCall(self, speech, language, regex):
        personToCall = re.findall(ur".*給(.*)",speech)
        if personToCall == []:
            personToCall = self.ask(u"請問要打給誰？")
        else:
            personToCall = personToCall[0]

        self.logger.info(u"####### {0} ##########".format(personToCall))
        ...
        if len(persons) > 0:
            if len(persons) == 1:
                ...
                while(personToCall == None):
                    ...
                    self.logger.info(u"####### {0} ##########".format(strUserToCall))
