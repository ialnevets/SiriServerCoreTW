...
class timePlugin(Plugin):
    ...
    localizations = {"currentTime": 
                        {"search":{"de-DE": "Es wird gesucht ...", "en-US":  u"請稍後 ..."},
                         "currentTime": {"de-DE": "Es ist @{fn#currentTime}", "en-US": u"現在是 @{fn#currentTime}"}},
                     "currentTimeIn": 
                        {"search":{"de-DE": "Es wird gesucht ...", "en-US": u"搜尋中"},
                         "currentTimeIn": 
                                {
                                "tts": {"de-DE": u"Die Uhrzeit in {0},{1} ist @{{fn#currentTimeIn#{2}}}:", "en-US": u"{1}, {0} 的時間為 @{{fn#currentTimeIn#{2}}}:"},
                                "text": {"de-DE": u"Die Uhrzeit in {0}, {1} ist @{{fn#currentTimeIn#{2}}}:", "en-US": u"{1}, {0} 的時間為 @{{fn#currentTimeIn#{2}}}:"}
                                }
                        },
                    "failure": {
    ...
    def currentTimeIn(self, speech, language):
        ...
        countryOrCity = re.findall(u"([\s\S]*)(現在幾點|的時間)",speech)[0][0]
        Query = urllib.quote_plus(str(countryOrCity.encode("utf-8")))
        if countryOrCity == None or countryOrCity == '':
        ...
        if countryOrCity != None:
            ...
            url = u"http://maps.googleapis.com/maps/api/geocode/json?address={0}&sensor=false&language=zh-TW".format(str(Query))
            ...
            if jsonString != None:
                ...
                if response['status'] == 'OK':
                    ...
                    if "country" in types:
                        ...
                        url = u"http://maps.googleapis.com/maps/api/geocode/json?address=capital%20{0}&sensor=false&language=zh-TW".format(urllib.quote_plus(components[0]['long_name']))
                            # lets wait max 3 seconds
                        jsonString = None
                        try:
                            jsonString = urllib2.urlopen(url, timeout=3).read()
                ...
                if response['status'] == 'OK':
                    ...
                    if jsonString != None:
                        ...
                        if "timezoneId" in timeZoneResponse:
                            ...
                            #timeZone = str(timeZone.encode("utf-8"))
                            #city = str(city.encode("utf-8"))
                            #country = str(country.encode("utf-8"))
                            #countryCode = str(countryCode.encode("utf-8"))
