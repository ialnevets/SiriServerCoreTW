...
class weatherPlugin(Plugin):
    ...
    def weatherForecastLookUp(self, speech, language):
        ...
        if countryOrCity != None:
            ...
            url = "http://maps.googleapis.com/maps/api/geocode/json?address={0}&sensor=false&language=zh-TW".format(urllib.quote_plus(countryOrCity.encode("utf-8")))
        elif countryOrCity == None:
            currentLocation=self.getCurrentLocation()
            url = "http://maps.googleapis.com/maps/api/geocode/json?latlng={0},{1}&sensor=false&language=zh-TW".format(str(currentLocation.latitude),str(currentLocation.longitude))
        ...
        if jsonString != None:
            ...
            if response['status'] == 'OK':
                ...
                if "country" in types:
                    ...
                    url = "http://maps.googleapis.com/maps/api/geocode/json?address=capital%20{0}&sensor=false&language=zh-TW".format(urllib.quote_plus(components[0]['long_name']))
                        # lets wait max 3 seconds
                    jsonString = None
                    try:
                        jsonString = urllib2.urlopen(url, timeout=3).read()
